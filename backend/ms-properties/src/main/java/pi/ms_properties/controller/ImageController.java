...
public class ImageController {
    ...
    public ResponseEntity<String> upload(@RequestParam("file") MultipartFile file, @RequestParam("propertyId") Long propertyId) throws IOException {
        String path = imageService.uploadImageToProperty(file, propertyId);
        return ResponseEntity.status(HttpStatus.CREATED).body("Se ha creado la imagen correctamente " + path);
    }
