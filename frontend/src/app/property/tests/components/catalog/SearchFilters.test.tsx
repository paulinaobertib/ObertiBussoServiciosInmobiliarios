...
describe("<SearchFilters />", () => {
  ...
  it("renderiza panel fijo en desktop y todos los acordeones básicos", () => {
    ...
    expect(screen.getByRole("button", { name: /Tipos de Propiedad/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Números de Ambientes/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /^Precio$/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Superficie \(Total \/ Cubierta\)/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Características/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Ciudades/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Barrios/i })).toBeInTheDocument();
  });
  ...
  it("tipos: marca check y llama toggleParam('types', name)", () => {
    ...
    renderWithTheme(<SearchFilters onSearch={onSearch} />);
    fireEvent.click(screen.getByRole("button", { name: /Tipos de Propiedad/i }));
  ...
  it("ambientes: permite marcar 1, 2, 3+", () => {
    ...
    renderWithTheme(<SearchFilters onSearch={onSearch} />);
    fireEvent.click(screen.getByRole("button", { name: /Números de Ambientes/i }));
  ...
  it("mobile controlado: onMobileOpenChange(true/false) se dispara al abrir/cerrar", () => {
    ...
    const { rerender } = renderWithTheme(
      <SearchFilters onSearch={onSearch} mobileOpen={false} onMobileOpenChange={onMobileOpenChange} />
    );
    fireEvent.click(screen.getByRole("button", { name: /Filtros/i }));
    ...
    rerender(
      <ThemeProvider theme={theme}>
        <SearchFilters onSearch={onSearch} mobileOpen={true} onMobileOpenChange={onMobileOpenChange} />
      </ThemeProvider>
    );
  ...
  it("hideMobileTrigger=true: en mobile NO muestra el botón 'Filtros'", () => {
    (useMediaQuery as any).mockReturnValue(true);
    renderWithTheme(<SearchFilters onSearch={onSearch} hideMobileTrigger />);
