  ...
  GridSection: (props: any) => {
    ...
    return (
      <div data-testid="grid-section">
        ...
        <button onClick={() => props.onCreate?.()} data-testid="btn-create">create</button>
        <button onClick={() => props.fetchAll?.()} data-testid="btn-fetch-all">fetchAll</button>
        <button onClick={() => props.fetchByText?.('')} data-testid="btn-search-blank">search-blank</button>
        <button onClick={() => props.fetchByText?.('ci')} data-testid="btn-search-term">search-term</button>
      </div>
    );
  },
}));
...
describe('CategorySection', () => {
  ...
  it('amenity: multi-select — hace diff y llama internalToggle en agregados y quitados', async () => {
    hookData = [{ id: 1, name: 'Pileta' }, { id: 2, name: 'Garage' }, { id: 3, name: 'Parrilla' }];
  ...
  it('type: columnas booleanas renderizan "Sí", "No" y "-" según corresponda', () => {
    hookData = [{
      id: 1, name: 'Casa',
      hasRooms: true, hasBedrooms: false, hasBathrooms: 'n/a', hasCoveredArea: undefined,
    }];
