...
import CategoryItems from '../../components/forms/CategorySection';
...
const mockusePropertiesContext = {
...
vi.mock('../../context/PropertiesContext', () => ({
  usePropertiesContext: () => mockusePropertiesContext,
}));
...
describe('CategoryItems', () => {
  ...
  it('no renderiza nada si no hay categoría', () => {
    mockusePropertiesContext.currentCategory = null;
    const { container } = render(<CategoryItems />);
    expect(container).toBeEmptyDOMElement();
    mockusePropertiesContext.currentCategory = 'amenity';
  });

  it('muestra loader cuando categoryLoading es true', () => {
    mockusePropertiesContext.categoryLoading = true;
    render(<CategoryItems />);
    expect(screen.getByRole('progressbar')).toBeInTheDocument();
    mockusePropertiesContext.categoryLoading = false;
  });

  it('muestra mensaje si no hay datos', () => {
    mockusePropertiesContext.data = [];
    render(<CategoryItems />);
  ...
  it('navega a los detalles de la propiedad al hacer clic en el botón "ver propiedad"', async () => {
    mockusePropertiesContext.currentCategory = 'property';
    mockusePropertiesContext.data = [
      {
        id: 1,
        title: 'Propiedad 1',
  ...
  it('muestra lista de items y columnas para categoría amenity', () => {
    ...
    mockusePropertiesContext.selected.amenities = [1];
  ...
  it('click en fila llama toggleSelect para no propiedad', () => {
    mockusePropertiesContext.currentCategory = 'amenity';
    mockusePropertiesContext.data = [{ id: 1, name: 'Amenity 1' }];
    mockusePropertiesContext.selected.amenities = [];
    render(<CategoryItems />);
    fireEvent.click(screen.getByText('Amenity 1').closest('div')!);
    expect(mockusePropertiesContext.toggleSelect).toHaveBeenCalledWith(1);
  });
  ...
  it('botón agregar navega si categoría es property', () => {
    ...
    mockusePropertiesContext.currentCategory = 'amenity'; // reset
  });
  ...
  it('eliminar propiedad llama deleteProperty y muestra alert', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockusePropertiesContext.refresh).toHaveBeenCalled();
    });
  });
  ...
  it('muestra SearchBarOwner solo si la categoría es "owner"', () => {
    mockusePropertiesContext.currentCategory = 'owner';
    mockusePropertiesContext.data = [];
  ...
  it('marca fila con fondo si está seleccionada', () => {
    mockusePropertiesContext.currentCategory = 'amenity';
    mockusePropertiesContext.data = [{ id: 99, name: 'Piscina' }];
    mockusePropertiesContext.selected.amenities = [99];
