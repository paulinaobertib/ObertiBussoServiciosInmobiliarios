...
describe("<ChartCard />", () => {
  ...
  it("renderiza un gráfico de tipo Pie", () => {
    renderWithTheme(<ChartCard title="Distribución" data={data} type="pie" />);
    expect(screen.getByTestId("chart-pie")).toBeInTheDocument();
    expect(h.lastPieProps.data.datasets[0].label).toBe("Distribución");
  });

  it("renderiza un gráfico de tipo Doughnut", () => {
    renderWithTheme(<ChartCard title="Anillo" data={data} type="doughnut" />);
    expect(screen.getByTestId("chart-doughnut")).toBeInTheDocument();
    expect(h.lastDoughnutProps.data.datasets[0].label).toBe("Anillo");
  });

  it("renderiza un gráfico de tipo Line", () => {
    renderWithTheme(<ChartCard title="Tendencia" data={data} type="line" />);
    expect(screen.getByTestId("chart-line")).toBeInTheDocument();
    expect(h.lastLineProps.data.datasets[0].borderColor).toBe("#42A5F5");
  });

  it("renderiza un gráfico de tipo Radar", () => {
    renderWithTheme(<ChartCard title="Radar" data={data} type="radar" />);
    expect(screen.getByTestId("chart-radar")).toBeInTheDocument();
    expect(h.lastRadarProps.data.datasets[0].label).toBe("Radar");
  });
});
