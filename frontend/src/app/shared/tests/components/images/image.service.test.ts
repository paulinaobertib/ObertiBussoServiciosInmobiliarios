...
describe("imageService", () => {
  ...
  describe("getImagesByPropertyId", () => {
    it("devuelve un array si la respuesta es válida", async () => {
      ...
      expect(mockedApi.get).toHaveBeenCalledWith("/properties/image/getByProperty/123", { withCredentials: true });
  ...
  describe("deleteImageById", () => {
    it("llama a la API con el ID correcto", async () => {
      ...
      expect(mockedApi.delete).toHaveBeenCalledWith("/properties/image/delete/789", { withCredentials: true });
    });
  });
});
