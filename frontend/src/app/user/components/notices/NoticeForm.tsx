...
export const NoticeForm = forwardRef<NoticeFormHandle, any>(({ initialData, onValidityChange }, ref) => {
  ...
  return (
    <Box component="form" noValidate onSubmit={(e) => e.preventDefault()}>
      <Grid container spacing={2} sx={{ alignItems: { xs: "flex-start", sm: "stretch" } }}>
        {/* Título */}
        <Grid size={{ xs: 12, sm: 8 }} sx={{ order: { xs: 1, sm: 1 }, minWidth: 0 }}>
          <TextField
            label="Título"
            fullWidth
            size="small"
        ...
        <Grid size={{ xs: 12, sm: 4 }} sx={{ order: { xs: 3, sm: 2 }, minWidth: 0 }}>
        ...
        <Grid size={{ xs: 12, sm: 8 }} sx={{ order: { xs: 2, sm: 3 }, minWidth: 0 }}>
          <TextField
            label="Descripción"
            fullWidth
            multiline
        ...
        <Grid size={{ xs: 12, sm: 4 }} sx={{ order: { xs: 4, sm: 4 }, minWidth: 0 }}>
          <Box
            sx={{
              width: "100%",
              borderRadius: 1,
          ...
          >
            <ImagePreview fullSizeSingle main={f.form.mainImage} images={[]} onDelete={() => f.setMain(null)} />
          </Box>
        </Grid>
      </Grid>
    </Box>
