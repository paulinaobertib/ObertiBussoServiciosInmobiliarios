    ...
    <Card elevation={1}>
      ...
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        <Typography variant="body2">
          ...
          {contract.lastPaidDate
            ? `${new Date(contract.lastPaidDate).toLocaleDateString()} - $${(
                contract.lastPaidAmount ?? 0
              ).toLocaleString()}`
            : "—"}
        </Typography>
      </CardContent>
        ...
        {isAdmin && (
          <>
            <Tooltip title="Servicios">
              <IconButton onClick={() => navigate(buildRoute(ROUTES.CONTRACT_UTILITIES, contract.id))}>
                <ElectricalServicesIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="Comisión">
              <IconButton onClick={() => navigate(buildRoute(ROUTES.CONTRACT_COMMISSION, contract.id))}>
                <ReceiptLongIcon />
              </IconButton>
            </Tooltip>
