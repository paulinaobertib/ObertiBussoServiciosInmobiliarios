...
export const AppointmentUserItem = ({ appointment, slot, onCancel, afterCancel }: Props) => {
  ...
  // Chip de estado
  const ChipBox = <Chip variant="outlined" label={label} size="small" color="primary" />;
  ...
  // Botón cancelar
  const CancelBtn = (
    <LoadingButton size="small" variant="outlined" color="error" loading={loading} onClick={handleCancel}>
      Cancelar
    </LoadingButton>
  );
