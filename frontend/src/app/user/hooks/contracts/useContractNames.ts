...
export function useContractNames(userId?: string, propertyId?: number | string) {
  ...
  useEffect(() => {
    ...
    getUserById(userId)
      ...
      .catch((e) => { console.error("user name error", e); alive && setUserName(""); });
  ...
  useEffect(() => {
    ...
    if (!pid || Number.isNaN(pid)) { setPropertyName(""); return; }
    ...
    getPropertyById(pid as number)
      ...
      .catch((e) => { console.error("property name error", e); alive && setPropertyName(""); });
