...
vi.mock("../../../../components/appointments/user/AppointmentUserItem", () => ({
  AppointmentUserItem: ({ appointment }: any) => <div data-testid={`appt-${appointment.id}`}>{appointment.id}</div>,
}));
...
describe("AppointmentUserList", () => {
    ...
    appointments = [
      {
        ...
        userId: "10",
        status: "ESPERA",
        availableAppointment: futureSlot,
  ...
  it("muestra mensaje si no hay turnos futuros", () => {
    render(<AppointmentUserList appointments={[]} slotMap={{}} onCancel={onCancel} reload={reload} />);
    expect(screen.getByText(/No hay turnos disponibles\./i)).toBeInTheDocument();
  });

  it("renderiza solo los turnos futuros", () => {
    render(<AppointmentUserList appointments={appointments} slotMap={slotMap} onCancel={onCancel} reload={reload} />);
