...
export const ChatSessionItem = React.memo(({ session, propertyTitle }: Props) => {
  ...
  return (
    <Card variant="outlined" sx={{ p: 2 }}>
      ...
      <Divider sx={{ mb: 2 }} />
      <Typography variant="body2">
        <strong>Fecha de consulta:</strong> {created.format("D [de] MMM YYYY, HH:mm")}
      </Typography>
      {isMobile ? <Box display="flex" flexDirection="column" gap={2} /> : <Box display="flex" alignItems="center" />}
      <Divider sx={{ mt: 2 }} />
      {ContactInfo}
    </Card>
  );
