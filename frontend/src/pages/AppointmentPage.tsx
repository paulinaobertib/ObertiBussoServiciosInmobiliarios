...
export default function AppointmentPage() {
  ...
  const stats = useMemo(() => {
    ...
    const espera = daySlots.filter((s) => !s.availability && apptsBySlot[s.id]?.status === "ESPERA").length;
    const aceptado = daySlots.filter((s) => !s.availability && apptsBySlot[s.id]?.status === "ACEPTADO").length;
    const rechazado = daySlots.filter((s) => !s.availability && apptsBySlot[s.id]?.status === "RECHAZADO").length;
          ...
          >
            ...
            >
              <CardContent sx={{ p: 2, display: "flex", flexDirection: "column", flexGrow: 1 }}>
                ...
                <Box sx={{ mt: "auto" }}>
                  <Button startIcon={<AddIcon />} variant="contained" fullWidth onClick={() => setGenerateOpen(true)}>
                    Generar turnos
                  </Button>
                </Box>
              </CardContent>
