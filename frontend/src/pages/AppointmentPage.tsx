        ...
        <>
            ...
            <BasePage>
                <Container sx={{ py: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, height: 'auto' }}>
                        {/* Card Izquierda */}
                        <Card sx={{ flexShrink: 0, flexBasis: { md: '350px' }, width: { xs: '100%', md: '350px' }, m: 1, display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
                            <CardContent sx={{ p: 2, display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                                ...
                                <Typography variant="subtitle1" align="center" fontWeight="bold" gutterBottom sx={{ mt: 2 }}>
                        ...
                        {/* Card Derecha */}
                        <Card sx={{ flexGrow: 1, m: 1, display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
                            <CardContent sx={{ p: 2, display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                                <Typography variant="subtitle1" align="center" fontWeight="bold" gutterBottom>
                                    Listado de Turnos
                                </Typography>
                    ...
                    <GenerateSlotsDialog open={generateOpen} onClose={() => setGenerateOpen(false)} />
                    <AppointmentDetailsDialog
                        open={detailsOpen}
                        slotId={selectedSlotId}
                        onClose={() => { setDetailsOpen(false); setSelectedSlotId(null); }}
                        onAccept={async (a) => { await acceptAppointment(a); reloadAdmin(); }}
                        onReject={async (a) => { await rejectAppointment(a); reloadAdmin(); }}
                        onDelete={async (id) => { await removeAvailableSlot(id); reloadAdmin(); }}
                    />
                </Container>
            </BasePage>
        </>
